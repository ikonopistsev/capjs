...
jsobj - формирование json объекта
...
Преобразования производятся добавлением окончания .[e] где e тип 
необходимого преобразования. Это работает для методов jsobj и jsarr.
Известные окончания удаляются при выдаче. Каждый тип имеет 
собственные преобразования. Всего mysql различает 
5 типов, плагин работает с 4мя.
...
jsarr - формирование массива json
    преобразования аналогичны jsobj 

jsd - конвертация миллисекунд в iso дату

mkkv - сформировать key=value
    select cast(
        mkkv("bob" as 'name')
    as char) as 'result';
    # result
